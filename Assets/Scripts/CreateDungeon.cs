...
public class CreateDungeon
{
    ...
    const int MinRoomHeight = 5;    // 部屋の最小の縦幅
    const int MaxRoomWidth = 30;    // 部屋の最大の横幅
    ...
    private void Create()
    {
        ...
        while(isConnectedAllRooms.Contains(false)){
            ...
            for(int i = 0; i < isConnectedAllRooms.Count; ++i){
        ...
        foreach(Path path in paths){
            ...
            // Debug.Log("(room1X, room1Y) = (" + room1X + ", " + room1Y + ")\n" + "(room2X, room2Y) = (" + room2X + ", " + room2Y + ")\n" + "room1Direction = " + room1Direction + "\n" + "room2Direction = " + room2Direction + "\n" + "room1PathStartPos = " + room1PathStartPos + "\n" + "room2PathStartPos = " + room2PathStartPos + "\n" + "PathCurvePos = " + path.PathCurvePos + "\n" + "room1Width = " + room1Width + "\n" + "room1Height = " + room1Height + "\n" + "room2Width = " + room2Width + "\n" + "room2Height = " + room2Height);
            if(path.PathCurvePos == 0){
                // 上下と左右でパスが繋がっている場合
    ...
    private List<Room> FindRoomsPosition(int k = 30){
        ...
        int firstRoomPositionX = Random.Range(0, MapWidth - MinRoomWidth);
        int firstRoomPositionY = Random.Range(0, MapHeight - MinRoomHeight);
        ...
        while(firstRoomPositionX + firstRoomWidth >= MapWidth || firstRoomPositionY + firstRoomHeight >= MapHeight || firstRoomWidth * firstRoomHeight >= MaxRoomArea){
            // 部屋が盤面の外に出る場合は再生成
            firstRoomPositionX = Random.Range(0, MapWidth - MinRoomWidth);
            firstRoomPositionY = Random.Range(0, MapHeight - MinRoomHeight);
        ...
        while(active.Count > 0){
            int centerIndex = Random.Range(0, active.Count);
    ...
    private Path MakePath(IReadOnlyList<Room> rooms, int room1Index, int room2Index){
        ...
        if(xRoomDirection == 0){
            ...
            if(yRoomDirection == 1){
                ...
                room1PathStartPos = Random.Range(0, rooms[room1Index].Size.x / 2) * 2;
                room2PathStartPos = Random.Range(0, rooms[room2Index].Size.x / 2) * 2;
                curvePos = Random.Range(1, yRoomDistance.Distance - 1);
            }else{
                // y方向でもう一つの部屋が上にある場合
            ...
            if(yRoomDirection == 0){
                ...
                room1PathStartPos = Random.Range(0, rooms[room1Index].Size.y / 2) * 2;
                room2PathStartPos = Random.Range(0, rooms[room2Index].Size.y / 2) * 2;
                curvePos = Random.Range(1, xRoomDistance.Distance - 1);
    ...
    private (List<Path>, List<List<RoomDistanceWithIndex>>) CreatePath(IReadOnlyList<Room> rooms){
        ...
        for(int line = 0; line < roomDistancesWithIndex.Count; ++line){
            for(int i = 0; i < roomDistancesWithIndex[line].Count; ++i){
