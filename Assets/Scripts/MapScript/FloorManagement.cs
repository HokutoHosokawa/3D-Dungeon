...
public class FloorManagement
{
    ...
    private readonly int _healObjectRoomIndex;           // 回復オブジェクトの部屋のインデックス
    private readonly Vector2Int _healObjectPosition;     // 回復オブジェクトの座標
    ...
    public FloorManagement(int floor, Material material)
    {
        ...
        _healObjectRoomIndex = Random.Range(CommonConst.MinIndex, _createDungeon.Rooms.Count - 1);
        if(_healObjectRoomIndex >= _floorClearRoomIndex)
        {
            // クリアルームと同じ部屋に回復オブジェクトがいないようにする
            _healObjectRoomIndex++;
        }
        Room healObjectRoom = _createDungeon.Rooms[_healObjectRoomIndex];
        _healObjectPosition = new Vector2Int(
            Random.Range(StairMinDistanceFromRoomEdge, healObjectRoom.Size.x - StairMinDistanceFromRoomEdge),
            Random.Range(StairMinDistanceFromRoomEdge, healObjectRoom.Size.y - StairMinDistanceFromRoomEdge)
        );
    ...
    public int HealObjectRoomIndex => _healObjectRoomIndex;
    public Vector2Int HealObjectPosition => _healObjectPosition;
